...
> **修訂 11 覆寫（2026-09-13）**：v0.4.1 完成逐筆串流顯示、同批搜尋去重、可取消的共用限流排程及跨背景重啟冷卻。新增 `storage` 只保存兩個限流時間。離線測試新增滑動窗口、延遲計時器、重啟、429、取消、儲存故障與 UI 串流；A6 仍需實機驗證首筆顯示、長批次、關閉／重開及實際 API 節奏，不得以模擬時序宣稱實站已通過。

> **修訂 10 覆寫（2026-09-13）**：v0.4.0 完整移除本機離線翻譯功能、wrapper、UI、測試與語言欄位，最低 Chrome 版本恢復為 114。產品保留批次 metadata、中文版本連結及欄位搜尋。A6 不再驗收語言包或翻譯，只需確認批次、搜尋連結、中文版本、錯誤隔離與不保存。
...
- 重新載入未封裝擴充功能，確認 manifest 顯示 0.4.0；提交一組 1–10 筆號碼後，確認每筆結果、單筆失敗隔離、標題／作者／社團搜尋連結、中文版本連結與不保存，且介面沒有翻譯區塊。
- 完成前不得標示最終 accepted。
