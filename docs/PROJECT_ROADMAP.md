...
> **修訂 6 覆寫（2026-09-12）**：使用者接受「中文版本連結＋本機機器直譯」新契約，並知情授權可能導向成人作品頁的 nhentai 連結。C1 已完成；I1、I2 已完成離線實作與品質閘門；目前進入 A6 Chrome 實機驗收。修訂 5 的 V3 NO-GO 只適用於「必須取得有來源中文名稱」的舊契約。
...
- 重新載入未封裝擴充功能，確認 manifest 顯示 0.3.0；第一次查詢觀察免費語言包下載，之後確認離線翻譯、作者原文、中文版本連結、無結果與不保存。
- 完成前不得標示最終 accepted。
