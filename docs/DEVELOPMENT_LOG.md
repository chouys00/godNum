...
## 2026-09-13｜修訂 11／v0.4.1 查詢速度與來源額度

- 使用者目標：優化按下查詢後到顯示結果的速度，並確保不超過來源 API 速率限制。
- 來源核對：唯讀取得官方 OpenAPI `2.0.0+483414f`，確認匿名 gallery 是 20／1min／IP、search 是 10／1min／IP；原有單一 20／分鐘假設不完整。只讀 API 規格，未發送作品／搜尋請求或取得作品內容。
- 實作：專屬 port 將來源 metadata 與中文版本逐筆傳回，補上版本時保留已顯示欄位及焦點；加入內部心跳、斷線收尾、關閉取消、單批重複訊息防護。同批相同搜尋重用前 25 筆配對必要欄位，仍逐作品核對，不跨批保存。
- 限流：共用序列排程採所有 API 至少 3.1 秒、search 至少 6.1 秒；依實際發送時間重新排程，計時器延遲不突波。429／503 支援 Retry-After 秒數／日期，至少 61 秒冷卻，不自動重試失敗項目；停用自動重新導向。
- 隱私：必要 `storage` 權限只寫入 `apiPacingV1.globalAt`／`searchAt` 兩個限流時間；請求前保守預留 61 秒，完成後保存正常節奏或較長冷卻，背景重啟不丟失限流。資料損壞／讀寫失敗不發送。未引入資料庫、外部依賴、網域、查詢歷史或內容保存。
- 量測：固定每次 API 回應 100ms，10 個不同搜尋共 20 次請求；首筆 metadata 100ms、首筆完整結果 3,200ms、整批 59,000ms。舊 UI 要等整批才顯示。三筆相同搜尋由 6 次 API 降為 4 次。這些是虛擬時鐘與 DOM 替身測試，非實機實站數值。
- 測試：新增 scheduler、worker 串流整合、side panel 串流測試，涵蓋 90 個混合並發排程的 60 秒窗口、慢回應／延遲計時器、重啟、429 長冷卻、無效標頭、待發／進行中取消、儲存故障、訊息來源、重複批次、多面板結果隔離、單筆失敗後繼續及 UI 焦點節點保留。`node test/run-tests.mjs` 與 `node test/run-quality-gate.mjs` 通過，51 個 JavaScript 檔案及七種違規 fixture 全數檢查。
- 文件：同步規格、roadmap、DEC-016、交接、驗收與補驗；新增 QUERY_PERFORMANCE.md。控制狀態仍由 STAGE_STATE.yaml 唯一管理，A6 維持 implementation_complete／CONCERNS，未標記最終 accepted。Chrome 實際時間、長批次與背景生命週期列入 SUP-A6-001；同 IP 的其他程式流量不受本擴充功能控制。
